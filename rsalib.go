...
func SignRsaPKCS1(Priv *rsa.PrivateKey, digest string) ([]byte, error) {
	...
	if _, err := pssNewHash.Write(message); err != nil {
		return []byte(""), err
	}
...
func GeneratePairKeys(path string, name string) error {
	...
	if PrivateKey, err := rsa.GenerateKey(rand.Reader, 2048); err == nil {
		...
		if err = SavePrivateKey(path + name + "PrivateKey.pem", PrivateKey); err == nil {
			if err = SavePublicKey(path + name + "PublicKey.pub", PublicKey); err != nil {
				return err
			}
		} else {
			return  err
...
/////////////////////////////////////////////////////////////////////////////////////////////////
func LoadPublicKey(strPubBase64 string) (*rsa.PublicKey, error) {

	if bytePub, err := base64.StdEncoding.DecodeString(strPubBase64); err == nil {
		if publicKey, err := x509.ParsePKCS1PublicKey([]byte(bytePub)); err == nil {
...
/////////////////////////////////////////////////////////////////////////////////////////////////
func LoadPrivateKey(strPrivBase64 string) (*rsa.PrivateKey, error) {

	if bytePriv, err := base64.StdEncoding.DecodeString(strPrivBase64); err == nil {
		if privateKeyFileImported, err := x509.ParsePKCS1PrivateKey(bytePriv); err == nil {
			return privateKeyFileImported, nil
		} else {
			return nil, err
		}
